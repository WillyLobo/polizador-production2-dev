...
| `LDAP_USER_FILTER` | `(sAMAccountName={1})` | Filtro de búsqueda. **Ojo, es `{1}` no `{0}`**: `GeoserverLdapBindAuthenticator` arma el filtro con `{username formateado por LDAP_USER_FORMAT, username crudo}` -- `{0}` sería `"IPDUV\globo"` (no matchea nada), `{1}` es `"globo"` (el que hay que comparar contra `sAMAccountName`). Confirmado en vivo (`IncorrectResultSizeDataAccessException: expected 1, actual 0` con `{0}`) |
| `LDAP_USER_FORMAT` | `<dominio NT4 de LDAP_BIND_DN>\{0}` (ej. `IPDUV\{0}`) | Patrón que GeoServer aplica al username ANTES de bindear -- **crítico**: `GeoserverLdapBindAuthenticator` bindea directo con el username ya formateado como principal (no hace search-then-bind-por-DN como Django); sin este campo bindea con el `sAMAccountName` pelado, que este AD rechaza (`Invalid credentials`, data `52e`) para cualquier usuario real aunque la contraseña sea correcta -- solo la cuenta admin local de GeoServer autentica. Confirmado en vivo contra el código fuente de `GeoserverLdapBindAuthenticator.java` |
