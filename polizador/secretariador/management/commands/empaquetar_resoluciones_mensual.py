...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # mismo mes, esto evita dejar un paquete-03.* huérfano de la corrida
        # vieja. Solo se tocan las extensiones que se están regenerando ahora.
        for blob_viejo in bucket.list_blobs(prefix=f"{prefijo_destino}/paquete-"):
            if blob_viejo.name.endswith(extensiones):
                blob_viejo.delete()

        for indice, paquete in enumerate(paquetes, start=1):
            if ".zip" in extensiones:
                destino = bucket.blob(f"{prefijo_destino}/paquete-{indice:02d}.zip")
                destino.upload_from_string(armar_zip(paquete), content_type="application/zip")
                self.stdout.write(f"  Armado {destino.name} ({len(paquete)} resoluciones)")
            if ".pdf" in extensiones:
                destino = bucket.blob(f"{prefijo_destino}/paquete-{indice:02d}.pdf")
                destino.upload_from_string(armar_pdf(paquete), content_type="application/pdf")
                self.stdout.write(f"  Armado {destino.name} ({len(paquete)} resoluciones)")
